...
TEST(EqualLoadBalancerTest, noFailuresTest) {
    ...
    auto newBlockRanges = loadBalancer.getNewBlocksAfterFailureForPushBlocks({});

    EXPECT_EQ(newBlockRanges.size(), 0);
}

TEST(EqualLoadBalancerTest, simpleLoadBalancerTestPushBlocks) {
    std::vector<std::pair<std::pair<ReStore::block_id_t, size_t>, ReStoreMPI::original_rank_t>> initialBlockRanges = {
        {{0, 100}, 0},   {{100, 100}, 1}, {{200, 100}, 2}, {{300, 100}, 3}, {{400, 100}, 4},
        {{500, 100}, 5}, {{600, 100}, 6}, {{700, 100}, 7}, {{800, 100}, 8}, {{900, 100}, 9}};

    auto loadBalancer   = ReStore::EqualLoadBalancer(initialBlockRanges, 10);
    auto newBlockRanges = loadBalancer.getNewBlocksAfterFailureForPushBlocks({0, 1});
    // don't commit previous change and discard it by requesting distribution for different set of dead ranks
    newBlockRanges = loadBalancer.getNewBlocksAfterFailureForPushBlocks({5, 7});
    ...
    newBlockRanges = loadBalancer.getNewBlocksAfterFailureForPushBlocks({0, 1});
...
TEST(EqualLoadBalancerTest, simpleLoadBalancerTestPullBlocks) {
    std::vector<std::pair<std::pair<ReStore::block_id_t, size_t>, ReStoreMPI::original_rank_t>> initialBlockRanges = {
        {{0, 100}, 0},   {{100, 100}, 1}, {{200, 100}, 2}, {{300, 100}, 3}, {{400, 100}, 4},
        {{500, 100}, 5}, {{600, 100}, 6}, {{700, 100}, 7}, {{800, 100}, 8}, {{900, 100}, 9}};
    ...
    std::vector<std::pair<std::pair<ReStore::block_id_t, size_t>, ReStoreMPI::original_rank_t>> newBlockRanges;
    for (ReStoreMPI::original_rank_t rank = 0; rank < 10; ++rank) {
        auto newBlockRangesPull = loadBalancer.getNewBlocksAfterFailureForPullBlocks({0, 1}, rank);
        // don't commit previous change and discard it by requesting distribution for different set of dead ranks
        newBlockRangesPull = loadBalancer.getNewBlocksAfterFailureForPullBlocks({5, 7}, rank);

        if (rank != 5 && rank != 7) {
            // This should be perfectly splittable into 25 block chunks
            EXPECT_EQ(newBlockRangesPull.size(), 1) << " on rank " << rank;
            std::transform(
                newBlockRangesPull.begin(), newBlockRangesPull.end(), std::back_inserter(newBlockRanges),
                [rank](auto request) { return std::make_pair(request, rank); });
        } else {
            // 5 and 7 are dead, so no data for them
            EXPECT_EQ(newBlockRangesPull.size(), 0);
        }
    }
