...
TEST(kMeansAlgorithm, constructor) {
    ...
    ASSERT_THROW(kMeansAlgorithm(generateRandomData<double>(0, 1), mpiContext, REPLICATION_LEVEL), std::invalid_argument);
    ASSERT_THROW(kMeansAlgorithm(generateRandomData<float>(0, 3), mpiContext, REPLICATION_LEVEL), std::invalid_argument);
    ...
    { // Constructing with an invalid data object should fail
        ...
        data << 2.5f;
        ASSERT_FALSE(data.valid());
        ASSERT_THROW((kMeansAlgorithm<float, MPIContext>(std::move(data), mpiContext, REPLICATION_LEVEL)), std::invalid_argument);
    }
    ...
    { // Constructing with a piecewise constructed kMeansData object should work
        ...
        ASSERT_TRUE(data.valid());
        ASSERT_NO_THROW((kMeansAlgorithm<float, MPIContext>(std::move(data), mpiContext, REPLICATION_LEVEL)));
    }

    { // Constructing with a valid, non-empty data object should work
        auto data1 = generateRandomData<double>(100, 30);
        ASSERT_NO_THROW((kMeansAlgorithm<double, MPIContext>(std::move(data1), mpiContext, REPLICATION_LEVEL)));

        auto data2 = kMeansData<double>(2, 2, 2);
        ASSERT_NO_THROW((kMeansAlgorithm<double, MPIContext>(std::move(data2), mpiContext, REPLICATION_LEVEL)));
    }

    { // Length of input vector not evenly dividible by the number of dimensions should fail
        ASSERT_THROW(
            (kMeansAlgorithm<float, MPIContext>(std::vector<float>{1, 2, 3}, 2, mpiContext, REPLICATION_LEVEL)), std::invalid_argument);
        ASSERT_THROW((kMeansAlgorithm<float, MPIContext>({1, 2, 3}, 2, mpiContext, REPLICATION_LEVEL)), std::invalid_argument);
    }

    { // Is fine
        ASSERT_NO_THROW((kMeansAlgorithm(std::vector<float>{1, 2, 3, 4}, 2, mpiContext, REPLICATION_LEVEL)));
        ASSERT_NO_THROW((kMeansAlgorithm<float, MPIContext>({1, 2, 3, 4}, 2, mpiContext, REPLICATION_LEVEL)));
    }
}
...
TEST(kMeansAlgorithm, pickCentersRandomly) {
    ...
    auto kmeansInstance = kMeansAlgorithm<float, MPIContext>(generateRandomData<float>(100, 3), mpiContext, REPLICATION_LEVEL);
...
TEST(kMeansAlgorithm, assignPointsToCenters) {
...

    {
        ...
        auto kmeansInstance = kMeansAlgorithm<float, MPIContext>(std::move(data), mpiContext, REPLICATION_LEVEL);
    ...
    {
        auto kmeansInstance = kMeansAlgorithm<float, MPIContext>({1, 2, 3, 7, 8, 9}, 1, mpiContext, REPLICATION_LEVEL);
...

    {
        ...
        auto kmeansInstance = kMeansAlgorithm<float, MPIContext>({0, 0, 1, 1, 2, 2, 6, 6, 7, 7, 9, 10}, 2, mpiContext, REPLICATION_LEVEL);
...
TEST(kMeansAlgorithm, smallExample) {
    ...
    auto kmeansInstance = kMeansAlgorithm<double, MPIContext>({1, 1, 2, 10, 13, 18, 20, 21}, 1, mpiContext, REPLICATION_LEVEL);
