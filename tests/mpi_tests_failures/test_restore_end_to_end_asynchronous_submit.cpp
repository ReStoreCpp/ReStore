...
TEST(ReStoreTest, EndToEnd_Simple1_PullBlocks) {
    ...
    store.submitBlocks(
        ...
        data.size() * static_cast<size_t>(numRanks()), // total number of blocks
        true                                           // asynchronous data exchange
    );
 
    // TODO: The asynchronous mode needs way more unit tests.
    store.pollSubmitBlocksIsFinished();
    store.waitSubmitBlocksIsFinished();
    ASSERT_TRUE(store.pollSubmitBlocksIsFinished());
